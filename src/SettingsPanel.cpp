...
void SettingsPanel::drawWorkspace(PanelActions& actions)
{
    ...
    if (visible && (resetLayout || !ImGui::DockBuilderGetNode(dockId)))
    {
        ...
        auto bottom =
            ImGui::DockBuilderSplitNode(center,
                                        ImGuiDir_Down,
                                        std::clamp(300 * uiScale / ImGui::GetWindowHeight(), 0.25f, 0.45f),
                                        nullptr,
                                        &center);
    ...
    if (visible && controlsOpen)
    {
        ...
        ImGui::TextUnformatted(
            "Navigate (viewport focused)\n  Left/right drag: look around\n  WASD / arrows: move; Q / E: up / "
        ...
        ImGui::Separator();
        ImGui::TextWrapped("Observer velocity changes aberration and measured light, not camera movement. "
...
void SettingsPanel::drawRenderSettings(PanelActions& actions)
{
    ...
    if (ImGui::Begin("Render Settings", nullptr, lockedLayout ? ImGuiWindowFlags_NoMove : 0))
    {
        ...
        if (ImGui::CollapsingHeader("Advanced"))
        {
            ...
            tip("Fixed radius integrates near the hole; full scene integrates everywhere; adaptive cutoff "
...
void SettingsPanel::drawCamera(PanelActions& actions,
                               double& slowStep,
    ...
    if (ImGui::Begin("Camera", nullptr, lockedLayout ? ImGuiWindowFlags_NoMove : 0))
    {
        ...
        if (ImGui::CollapsingHeader("Observer physics", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            for (int axis = 0; axis < 3; ++axis)
            {
                ...
                ImGui::SetNextItemWidth(-1);
                if (ImGui::SliderScalar("##velocity",
                                        ImGuiDataType_Double,
                                        &value,
                                        &minimum,
                                        &maximum,
                ...
                draw->AddLine(
                    {mid, right.y - 4 * uiScale}, {mid, right.y}, ImGui::GetColorU32(ImGuiCol_TextDisabled));
            ...
            for (auto sphere : scene.spheres)
                if (scene.materials[sphere.material.x].kindTexture.x == Schwarzschild)
                {
                    ...
                    if (radius <= 1 && draft.observerType == RenderSettings::Hovering)
                        ImGui::TextWrapped("Hovering is undefined at or inside the horizon. Choose Freely "
...
void SettingsPanel::drawViewport(const PanelActions& actions,
                                 const CameraData& camera,
                                 const SceneData& scene,
                                 double aspect)
{
                                 ...
                                 const SceneData& scene)
{
    ...
    if (visible)
    {
        ...
        ImGui::TextDisabled("%s  |  %d x %d  |  %.1f / %d samples",
                            ...
                            active.width,
                            active.height,
                            progress.meanSamples,
                            active.samples);
        if (ImGui::IsItemHovered())
            ImGui::SetTooltip("GPU batch: %.2f ms | invalid rays: %llu",
                              progress.lastBatchMilliseconds,
