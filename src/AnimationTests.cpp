...
namespace rt
{
    namespace
    {
        ...
        void verifyVideo(const std::filesystem::path &path, int width, int height, int frames, int fps)
        {
            ...
            for (;;)
            {
                ...
                hr(reader->ReadSample(DWORD(MF_SOURCE_READER_FIRST_VIDEO_STREAM), 0, nullptr, &flags,
                                      &timestamp, &sample));
                if (sample)
                {
                    ...
                    if (!count)
                    {
                        ...
                        bool correct = bottom + 2 < length && top + 2 < length &&
                                       bytes[top + 2] > bytes[top] + 100 &&
            ...
            hr(reader->GetPresentationAttribute(DWORD(MF_SOURCE_READER_MEDIASOURCE), MF_PD_DURATION,
                                                &duration));
    ...
    int runAnimationTests()
    {
        ...
        require(tall.x == 350 && tall.width == 300 && tall.height == 600,
                "Portrait images must be pillarboxed");
        ...
        require(e.clip.tracks[1].evaluate(-1).position.x == 2 &&
                    e.clip.tracks[1].evaluate(99).position.x == 12,
                "Endpoint holds");
