...
{
    camera.basis(1);
    Vec3 z = normalized(camera.position - camera.lookAt);
    ...
    else
    {
        ...
        if (m[2][2] > m[i][i])
            i = 2;
        int j = (i + 1) % 3, k = (i + 2) % 3;
        double s = 2 * std::sqrt(1 + m[i][i] - m[j][j] - m[k][k]);
        double v[3]{};
        v[i] = s / 4;
        v[j] = (m[j][i] + m[i][j]) / s;
        v[k] = (m[k][i] + m[i][k]) / s;
...
Pose interpolate(const Pose& a, const Pose& b, double t)
{
    ...
    if (d < 0.9995)
    {
        double theta = std::acos(std::clamp(d, -1.0, 1.0));
...
bool samePose(const Pose& a, const Pose& b)
{
    ...
    double c = std::abs(p.x * q.x + p.y * q.y + p.z * q.z + p.w * q.w);
    return dot(d, d) < 1e-20 && std::abs(1 - c) < 1e-12;
}

int AnimationTrack::keyAt(int frame) const
{
    auto it = std::lower_bound(keys.begin(),
                               keys.end(),
                               frame,
                               [](const Keyframe& k, int f)
                               {
                                   return k.frame < f;
                               });
    return it != keys.end() && it->frame == frame ? int(it - keys.begin()) : -1;
}

Pose AnimationTrack::evaluate(int frame) const
{
    if (keys.empty())
        return base;
    ...
    const auto& a = *(it - 1);
    return interpolate(a.pose, it->pose, double(frame - a.frame) / (it->frame - a.frame));
}

std::vector<Pose> AnimationClip::evaluate(int frame) const
{
...
void AnimationClip::apply(int frame, SceneData& scene, CameraData& camera, bool drafts) const
{
    for (const auto& t : tracks)
    {
        ...
        else
        {
            ...
            c.x = float(p.position.x);
            c.y = float(p.position.y);
            c.z = float(p.position.z);
        }
    }
}
...
AnimationEditor::AnimationEditor(const SceneData& scene, const CameraData& camera)
{
    ...
    for (int i = 0; i < int(scene.spheres.size()); ++i)
    {
        ...
        int kind = scene.materials[s.material.x].kindTexture.x;
        if (kind == Environment)
            continue;
        std::string name = kind == Earth           ? "Earth"
                           : kind == Moon          ? "Moon"
...
void AnimationEditor::observe(const SceneData& scene, const CameraData& camera)
{
    for (auto& t : clip.tracks)
    {
        ...
        if (t.body >= 0)
            expected.position = {
                float(expected.position.x), float(expected.position.y), float(expected.position.z)};
...
void AnimationEditor::seek(int destination)
{
    endGesture();
    if (hasDrafts())
    {
...
void AnimationEditor::capture()
{
    ...
    Pose p = t.draft ? *t.draft : t.evaluate(frame);
    int k = t.keyAt(frame);
    if (k >= 0)
        t.keys[k].pose = p;
    else
...
bool AnimationEditor::configure(int frames, int fps)
{
