...
namespace rt
{
    ...
    struct GpuRenderer::Impl
    {
        ...
        void buffer(int binding, const void* data, std::size_t bytes) const
        {
            if (bytes > static_cast<std::size_t>(maxStorage))
        ...
        void integer(const char* name, int v) const
        {
    ...
    GpuRenderer::GpuRenderer(const std::filesystem::path& assets) : impl(new Impl)
    {
        ...
        const auto marker = source.find("// TRACE_CORE");
