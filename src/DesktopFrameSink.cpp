...
class MediaFoundationWriter final : public VideoWriter
{
  ...
  public:
    ...
    {
        checkHr(MFCreateSinkWriterFromURL(path.c_str(), nullptr, nullptr, &writer), "Create MP4 writer");
        ...
        checkHr(MFSetAttributeSize(output.Get(), MF_MT_FRAME_SIZE, s.width, s.height), "Set output size");
        checkHr(MFSetAttributeRatio(output.Get(), MF_MT_FRAME_RATE, s.fps, 1), "Set output FPS");
        checkHr(MFSetAttributeRatio(output.Get(), MF_MT_PIXEL_ASPECT_RATIO, 1, 1), "Set pixel aspect");
        ...
        checkHr(input->SetUINT32(MF_MT_DEFAULT_STRIDE, UINT32(-s.width * 4)), "Set bottom-up RGB stride");
        checkHr(MFSetAttributeSize(input.Get(), MF_MT_FRAME_SIZE, s.width, s.height), "Set input size");
        checkHr(MFSetAttributeRatio(input.Get(), MF_MT_FRAME_RATE, s.fps, 1), "Set input FPS");
        checkHr(MFSetAttributeRatio(input.Get(), MF_MT_PIXEL_ASPECT_RATIO, 1, 1), "Set input pixel aspect");
...
class FfmpegWriter final : public VideoWriter
{
    ...
    static SDL_Process* launch(const std::vector<std::string>& args)
    {
        ...
        if (!p)
            throw std::runtime_error(std::string("Cannot launch ffmpeg from PATH: ") + SDL_GetError());
        return p;
    }

  public:
    FfmpegWriter(const ExportSpec& s, const std::filesystem::path& path, const std::atomic<bool>& cancelled)
    {
        ...
        probe = launch({"ffmpeg",
                        ...
                        "lavfi",
                        "-i",
                        "color=black:size=" + std::to_string(s.width) + "x" + std::to_string(s.height) +
                            ":rate=" + std::to_string(s.fps),
                        "-frames:v",
                        "1",
                        "-an",
                        "-c:v",
...
class DesktopFrameSink final : public FrameSink
{
...
#ifdef _WIN32
                if (!MoveFileExW(
                        (temporary / L"video.mp4").c_str(), spec.destination.c_str(), MOVEFILE_WRITE_THROUGH))
                    throw std::runtime_error(
  ...
  public:
    ...
    {
        std::lock_guard<std::mutex> lock(mutex);
        if (status.state != SinkState::Ready || queue.size() >= 2 || finishRequested || cancelRequested)
            return false;
